/*
- `possibleStates` is an object whose keys refer to the state name and whose values are instances of the `State` class (or subclasses). 
    {
        idle: new IdleState(),
        move: new MoveState(),
...
The class assigns the `stateMachine` property on each instance so they can call `this.stateMachine.transition` whenever they want to trigger a transition.
...
class StateMachine {
    constructor(initialState, possibleStates, stateArgs=[]) {
        ...
        this.collision_arrow = false;
        this.collision_orb = false;
    }
